...
________________
 ...
 إدا حاول أحد  طرد الكل أو مسح الرومات والشات يتدخل البوت ويمنعه ويخبر الاونر برسالة في الخاص 
...
client.on('message', message => {
    var prefix = "₩";
      ...
      if (message.author.id == 396851041494237186) return;


    if (message.content.startsWith(prefix + 'playing')) {
    if (message.author.id !== '396851041494237186') return message.reply('** هذا الأمر فقط لصاحب البوت و شكراًً **')
...
  const size = channel.name.match(/\[\s(\d+)\s\]/);
  if (!size) return channel.setName(`عدد المتصلون: ${currentSize}`);
  if (currentSize !== size) channel.setName(`عدد المتصلون: ${currentSize}`);
});
