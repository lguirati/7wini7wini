...
client.on('guildMemberAdd', member => {
     const welcomer =  member.guild.channels.find('name', '🔻ʟᴏʀᴅ-ᴡᴇʟᴄᴏᴍᴇ');
const w = ['./w1.png'];
...
client.on('voiceStateUpdate', (old, now) => {
  ...
  if (!size) return channel.setName(`ᴠᴏɪᴄᴇ ᴏɴʟɪɴᴇ: ${currentSize}`);
  if (currentSize !== size) channel.setName(`ᴠᴏɪᴄᴇ ᴏɴʟɪɴᴇ: ${currentSize}`);
});
